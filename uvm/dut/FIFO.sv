...
module FIFO #(
  ...
  parameter ADDR_WIDTH = $clog2(WORD_WIDTH) + 1     // MSB - loop ; Remaining address value
)(
...
always @(posedge ClkWrite or posedge reset) begin
  if (reset == 1) begin
    ...
    if (!(ptrWriteBin != syncReadBin && ptrWriteBin[ADDR_WIDTH-2 : 0] == syncReadBin[ADDR_WIDTH-2 : 0])) begin
      ...
      if (((ptrWriteBin[ADDR_WIDTH-2 : 0] + 3'b010) != syncReadBin[ADDR_WIDTH-2 : 0]) && ((ptrWriteBin[ADDR_WIDTH-2 : 0] + 3'b001) != syncReadBin[ADDR_WIDTH-2 : 0])) begin
        haltInputDrive <= 0;                                                             
      end
    end
  end 
...
always @(posedge ClkWrite) begin
    if (ControlIn && !HaltInput) begin
        $display("WRITE @%0t  data=%h", $time, DataIn);
    end
end
  
always @(posedge ClkRead) begin
    if (ReadValid) begin
        $display("READ  @%0t  data=%h", $time, DataOut);
    end
end
  
always @(posedge ClkWrite) begin
    $display("WCLK %0t | Wptr=%0d Rptr_sync=%0d | HaltInput=%b",
             $time, ptrWriteBin, syncReadBin, HaltInput);
end
  
always @(posedge ClkWrite) begin
  $display("MEM: [%h] [%h] [%h] [%h] [%h] [%h] [%h] [%h]",
...
initial begin
    #200000;
    $display("WATCHDOG: Ending simulation");
    $finish;
end
*/
